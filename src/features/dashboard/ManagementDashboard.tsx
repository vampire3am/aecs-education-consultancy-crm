...
import { CalendarDays, CheckCircle2, UserPlus, Users, XCircle } from "lucide-react";
import { isSupabaseConfigured, supabase } from "../../lib/supabase";

type Datum = { name: string; value: number };
type Stats = { total:number; today:number; converted:number; closed:number; trend:Datum[]; countries:Datum[] };
...
export function ManagementDashboard() {
  const [stats,setStats] = useState<Stats>({total:0,today:0,converted:0,closed:0,trend:[],countries:[]});
  ...
  ]).then(([total,today,converted,closed,records]) => {
    const months = new Map<string,number>(); const countries = new Map<string,number>();
    (records.data || []).forEach((record:any) => { const key = new Date(record.created_at).toLocaleDateString("en",{month:"short",year:"2-digit"}); months.set(key,(months.get(key)||0)+1); const country=record.study_preferences?.preferred_country||"Other"; countries.set(country,(countries.get(country)||0)+1); });
    setStats({total:total.count||0,today:today.count||0,converted:converted.count||0,closed:closed.count||0,trend:[...months].map(([name,value])=>({name,value})).slice(-8),countries:[...countries].map(([name,value])=>({name,value})).sort((a,b)=>b.value-a.value)});
  }); },[]);
  const cards = [["Total leads",stats.total,Users,"blue"],["Today's registrations",stats.today,UserPlus,"amber"],["Converted to student",stats.converted,CheckCircle2,"green"],["Closed leads",stats.closed,XCircle,"violet"]] as const;
